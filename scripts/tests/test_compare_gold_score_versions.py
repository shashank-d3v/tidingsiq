...
class CompareGoldScoreVersionsTest(unittest.TestCase):
    def test_overlap_summary_sql_counts_overlap_and_diff(self) -> None:
        sql = build_overlap_summary_sql(
            ...
            "example-project.gold.positive_news_feed_v3_shadow",
        )
    ...
    def test_distribution_and_precision_queries_reference_expected_columns(self) -> None:
        self.assertIn(
            ...
            build_exclusion_distribution_sql("example-project.gold.positive_news_feed"),
        )
        self.assertIn(
            "source_domain",
            build_domain_mix_sql(
        ...
        self.assertIn(
            "url_quality_status",
            build_broken_link_mix_sql("example-project.gold.positive_news_feed_v3_shadow"),
        )
        self.assertIn(
            "include_precision",
            build_label_precision_sql(
                "example-project.gold.positive_news_feed_v3_shadow",
                "example-project.gold.scoring_eval_labels",
            ),
        )
